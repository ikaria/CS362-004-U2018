 ...
 * both are tested.
 *
 * void newSmithy(struct gameState *state, int currentPlayer);
 *
 * This function handles the playing of the newSmithy function, which is known to have a bug. The function adds three cards
 * to a players hand but otherwise makes no changes to the game state
 *
...
// Put the village card at a random position in the hand
        ...
        G.hand[p][cardPosition]=smithy;  // Maybe not necessary
...
// Deck count should be decreased by three
        testResult += testAssert(G.deckCount[p], Gcopy.deckCount[p]-3, &totalTestCount);
        if (testResult <= -1) {
